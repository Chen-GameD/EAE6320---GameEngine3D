...
eae6320::cResult eae6320::Graphics::cMesh::CreateMeshWithBinaryFile(cMesh*& o_mesh, const char* const i_path)
{
	auto result = Results::Success;

	// Create mesh needed data
	...
	// Sign all data to 'Create mesh needed data'
	vertexCount = *reinterpret_cast<uint16_t*>(buffer);
	vertexData = reinterpret_cast<eae6320::Graphics::VertexFormats::sVertex_mesh*>(buffer + sizeof(uint16_t));
	indexCount = *reinterpret_cast<uint16_t*>(buffer + sizeof(uint16_t) + sizeof(eae6320::Graphics::VertexFormats::sVertex_mesh) * vertexCount);
	indexArray = reinterpret_cast<uint16_t*>(buffer + sizeof(uint16_t) + sizeof(eae6320::Graphics::VertexFormats::sVertex_mesh) * vertexCount + sizeof(uint16_t));
